...
int main()
 {
	...
	std::string OPfolder;
	std::string copyfoldercmd;

	reenterDP:
	std::cout << "Enter the filepath to a directory containing ONLY the .txt data files from a logger" << std::endl;
	std::getline(std::cin,sourcefolder);

	if (dirExists(sourcefolder) == FALSE) {
		std::cout << "\n";
		std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
		std::cout << "\n";
		std::cout << "This directory does not exist =(.... Try again\n";
		std::cout << "\n";
		std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
		std::cout << "\n";
		goto reenterDP;
	...
	copyfoldercmd = "\"" + sourcefolder + "\\MERGED_TEMP12151\"";
	std::string temps = "if not exist " + copyfoldercmd + " mkdir " + copyfolder;

	char* cmd = new char[temps.length() + 1];
	strcpy(cmd, temps.c_str());
	//char cmd[150];
	//std::strcpy(array, temps.c_str());
	system(cmd);
	system("pause");
	...
	char* cmd2 = new char[src2cpy.length() + 1];
	strcpy(cmd2, src2cpy.c_str());
	...
	// "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
	//						NEED TO FIX FILEPATH TO DEAL WITH WHITE SPACE
	//"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
	// "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
	...
	char* mkOP = new char[temps2.length() + 1];
	strcpy(mkOP, temps2.c_str());
	system(mkOP);
...
//	std::strcpy(mvOP, cpy2OP.c_str());
	...
	char* mvOP = new char[cpy2OP.length() + 1];
	strcpy(mvOP, cpy2OP.c_str());
	system(mvOP);
...
//	char delc[150];
	...
	char* delc = new char[delcpy.length() + 1];
	strcpy(delc, delcpy.c_str());
	system(delc);
	...
	// open DO.temp and exit the program
	std::string DOTEMP = OPfolder + "\\DOTemp.png";
	//char temp3[150];
//	std::strcpy(temp3, DOTEMP.c_str());
	char* openDOTEMP = new char[DOTEMP.length() + 1];
	strcpy(openDOTEMP, DOTEMP.c_str());
	system(openDOTEMP);
